...
*/
/*$SQL = "SELECT id,Name,Pic_Url, X,Y, ( 1.609344 * acos( cos( radians(39.922478) ) * cos( radians($Y) ) *
		cos( radians( $X ) - radians(116.443710) ) + sin( radians(39.922478) ) *
		sin( radians( $Y ) ) ) ) AS distance FROM lists HAVING
		distance < 25 ORDER BY distance LIMIT 0 , 500;";*/
...
class BusinessList extends CI_Model{
	...
	public function records($id=NULL,$business_id=NULL,$show = NULL){
		...
		if($show =="Detail"){
				$query = $this->db->query("SELECT *  FROM lists WHERE user_id='".$id."' AND business_id='".$business_id."' ORDER BY business_id asc");
		}else{
			if($id==NULL){$query = $this->db->query("SELECT * FROM businesses WHERE business_status=1 ORDER BY business_id asc");}
			else{$query = $this->db->query("SELECT *  FROM businesses WHERE user_id='".$id."' ORDER BY business_id asc");}
		}
	...
	public function rdata(){
		//$Y = '32.10933';//$this->input->post('lat');
		//$X = '34.855499';//$this->input->post('lng');
		//$Y = '16.6104339';
		//$X = '120.3147096';
		//16.616899, 120.315554
		//16.616899, 120.315554
	$Y = $this->input->post('lat');
	$X = $this->input->post('lng');

		if($this->input->post('type')=="searchbyname"){
			$id = $this->input->post('ID');
			//$SQL = "SELECT id,Name,Pic_Url, X,Y,ShortDescription,City,Phone,(6371  * acos (cos ( radians($Y) ) * cos( radians( $Y ) ) * cos( radians( $X ) - radians($X) ) + sin ( radians($Y) ) * sin( radians( $Y ) )) ) AS distance FROM lists HAVING distance < 30 AND id = $id ORDER BY distance LIMIT 0 , 40;";
			//SELECT*, ( 6371 * acos( cos( radians(-6.811) ) * cos( radians( latitude ) ) * cos( radians( longitude ) - radians(110.851) ) + sin( radians(-6.811) ) * sin( radians( latitude ) ) ) ) AS distance FROM markers HAVING distance < 100 ORDER BY distance LIMIT 0 , 20
		}else{
			//$SQL = "SELECT id,Name,Pic_Url, X,Y,ShortDescription,City,Phone,( 3959  * acos ( cos ( radians($Y) ) * cos( radians( Y ) ) * cos( radians( $X ) - radians(X) ) + sin ( radians($Y) ) * sin( radians(Y) ) ) ) AS distance FROM lists HAVING distance < 30 ORDER BY distance LIMIT 0 , 40;";
			//$SQL ="SELECT id,Name,Pic_Url, X,Y,ShortDescription,City,Phone, ( 6371 * acos( cos( radians($Y) ) * cos( radians( Y ) ) * cos( radians( X ) - radians($X) ) + sin( radians($X) ) * sin( radians( Y ) ) ) ) AS distance FROM  lists HAVING distance < 100 ORDER BY distance LIMIT 0 , 20";
			$SQL ="SELECT ROUND(3963.0 * ACOS(SIN( $Y*PI()/180 ) * SIN( $Y*PI()/180 ) + COS( $Y*PI()/180 ) * COS( $Y*PI()/180 ) * COS( (Y*PI()/180) - ($X*PI()/180) ) ) , 1) AS distance, id,Name,Pic_Url, X,Y,ShortDescription,City,Phone FROM lists HAVING distance < 10 ORDER BY distance ASC LIMIT 0, 10";
		}
	...
	public function businesslike($name){
		$query = $this->db->query("SELECT id,name  FROM lists WHERE Name LIKE '%$name%' ORDER BY id LIMIT 0 , 20;");
		return json_encode($query->result());
