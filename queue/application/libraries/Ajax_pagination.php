...
class Ajax_pagination{
	...
	function create_links(){
		...
		//}
			$output .=  $this->getAJAXlink( $i, $this->prev_link );
		...
		$output .='<a href="javascript:void(0);" class="btn btn-primary aligncenter" onclick="skipfunction('.$this->ID.');"  style="width:220px; padding:10px 50px; font-size:1em; margin:10px;">SKIP</a>';
		// Render the "next" link
		//if ($this->cur_page < $num_pages){
			$output .= $this->getAJAXlink( $this->cur_page * $this->per_page , $this->next_link );
		...
		function skipfunction(ID){
		
			$.ajax({
                method: "POST",
                url: "<?php echo base_url(); ?>/queue/skip",
                data: { ID: ID },
                success: function(data){
	...
	function getAJAXlink($count, $text) {
		...
		return '<input type="submit" ' .   'onclick="'.$this->link_func.'('.$pageCount.')" class="next btn btn-primary" value="'.$text. '" style="width:220px; padding:10px 50px; font-size:1em; margin:10px;">';
	}
